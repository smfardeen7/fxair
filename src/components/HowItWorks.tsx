          ...
          {steps.map((item, i) => (
            <div key={item.step} className="relative">
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm relative z-10">
                <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-fxair-orange text-white font-display font-bold text-lg">
                  {item.step}
                </span>
                <h3 className="font-display font-semibold text-fxair-gray-dark mt-4 mb-2">{item.title}</h3>
                <p className="text-sm text-fxair-gray leading-relaxed">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
